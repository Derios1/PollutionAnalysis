...
class TimeSeriesCV(abc.ABC):
    def __init__(self, estimator, test_size, fig, axs, param_grid):
    ...
    def make_plot(self, errors):
        ...
        for h_i in range(self.test_size):
            for ax, error in zip(self.axs, errors[h_i]):
                ax.plot(range(n), error, color=colors[h_i])
...
class TimeSeriesWindowCV(TimeSeriesCV):
    ...
    def __init__(self, estimator, train_size, test_size, shift, fig, axs, param_grid=None):
        super().__init__(estimator, test_size, fig, axs, param_grid)
    ...
    def make_forecasts(self, X, Y, metric, params):
        ...
        errors, i = np.zeros(shape=(self.test_size, (len(X) - window_size - self.test_size) // self.shift + 1, 3)), 0
...
class TimeSeriesWalkingForwardCV(TimeSeriesCV):
    def __init__(self, estimator, test_size, n_splits, fig, axs, gap=0, param_grid=None):
        super().__init__(estimator, test_size, fig, axs, param_grid)
